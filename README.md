...
```bash
AWSLambdaTodoJavaAPI$ sam build
```
...
```bash
AWSLambdaTodoJavaAPI$ sam local invoke HelloWorldFunction --event events/event.json
```

The SAM CLI can also emulate your application's API. Use the `sam local start-api` to run the API locally on port 3000.

```bash
AWSLambdaTodoJavaAPI$ sam local start-api
AWSLambdaTodoJavaAPI$ curl http://localhost:3000/
```
...
```bash
AWSLambdaTodoJavaAPI$ sam logs -n HelloWorldFunction --stack-name AWSLambdaTodoJavaAPI --tail
```
...
```bash
AWSLambdaTodoJavaAPI$ cd HelloWorldFunction
...
```bash
sam delete --stack-name AWSLambdaTodoJavaAPI
```
